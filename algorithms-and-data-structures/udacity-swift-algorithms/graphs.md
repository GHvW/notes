...
class Graph {
    ...
    func getEdgeList() -> [[Int]] {
        return edges.map { edge in
            var arr = [Int]()
            if let val = edge.value { arr.append(val) }
            if let from = edge.nodeFrom?.value { arr.append(from) }
            if let to = edge.nodeTo?.value { arr.append(to) }
    ...
    func getAdjacencyList() -> [[[Int]]] {
        let max = getMaxIndex()
        var adjacencyList: [[[Int]]] = Array(repeating: [], count: max + 1)
        
        for edge in edges {
            let from = edge.nodeFrom?.value ?? 0
            var temp = [Int]()
            if let to = edge.nodeTo?.value { temp.append(to) }
            if let val = edge.value { temp.append(val) }
            adjacencyList[from].append(temp)
        }
    ...
    func getAdjacencyMatrix() -> [[Int]] {
        let max = getMaxIndex()
        var adjacencyMatrix = Array(repeating: Array(repeating: 0, count: max + 1), count: max + 1)
        
        for edge in edges {
            let from = edge.nodeFrom?.value ?? 0
            let to = edge.nodeTo?.value ?? 0
            guard let val = edge.value else { continue }
